...
def register():
    if request.method == 'POST':
        ...
        r = radist.Radist(api_key=radist_api_key, connection_id=int(radist_connection_id))
        ...
        if not status:
            flash(text)
            return render_template('register.html')
        database.add_search_account(radist_api_key, radist_connection_id, amo_login, amo_host, amo_password,
                                    hi_message='Привет', email=username, password=password)
