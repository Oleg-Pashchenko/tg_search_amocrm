...
def login_required(view_func):
    ...
    def wrapped_view(*args, **kwargs):
        if "user_id" not in session:
            flash(
                "Пожалуйста, войдите в систему для доступа к этой странице.", "warning"
            )
            return redirect("/")
...
def register():
    if request.method == "POST":
        ...
        password = request.form["password"]
        radist_api_key = request.form["radist_api_key"]
        radist_connection_id = request.form["radist_connection_id"]
        amo_login = request.form["amo_login"]
        amo_host = request.form["amo_host"]
        amo_password = request.form["amo_password"]
        ...
        r = radist.Radist(
            api_key=radist_api_key, connection_id=int(radist_connection_id)
        )
        text, status = r.connect()
        if not status:
            flash(text)
            return render_template("register.html")

        a = amocrm.Amocrm(host=amo_host, email=amo_login, password=amo_password)
        text, status = a.connect()
        print(text, status)
        if not status:
            flash(text)
            return render_template("register.html")
        database.add_search_account(
            radist_api_key,
...
def home():
    if request.method == "POST":
        ...
        database.update_search_info(
            params["search_keywords"],
            params["hi_message"],
            params["account_to_post"],
            session["user_id"],
        )
...
def chats():
    ...
    for chat in chat_list:
        database.add_telegram_chat(
            session.get("user_id"), chat["id"], chat["title"], False
        )
    return redirect("/home")
